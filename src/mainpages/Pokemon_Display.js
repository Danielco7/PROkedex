...
function Pokemon_Display(match) {
  ...
  useEffect(() => {
    async function getPokemon() {
      ...
      if (data3.chain.evolves_to.length != 0) {
        if (
          data3.chain.evolves_to[0].species.url.slice(
            42,
            data3.chain.evolves_to[0].species.url.length - 1
          ) < 152
        ) {
          await setpokemonchain((oldArray) => [
            ...oldArray,
            data3.chain.species.name,
          ]);
          await setpokemonchainlvl((oldArray) => [
            ...oldArray,
            data3.chain.evolves_to[0].evolution_details[0].min_level,
          ]);
          await setpokemonchain((oldArray) => [
            ...oldArray,
        ...
        if (data3.chain.evolves_to[0].evolves_to.length != 0) {
          if (
            data3.chain.evolves_to[0].evolves_to[0].species.url.slice(
              42,
              data3.chain.evolves_to[0].evolves_to[0].species.url.length - 1
            ) < 152
          ) {
            await setpokemonchain((oldArray) => [
              ...oldArray,
              data3.chain.evolves_to[0].evolves_to[0].species.name,
            ]);
            await setpokemonchainlvl((oldArray) => [
              ...oldArray,
              data3.chain.evolves_to[0].evolves_to[0].evolution_details[0]
                .min_level,
            ]);
          }
        }
      ...
      for (let i = 0; i < data.moves.length; i++) {
        ...
        if (element.version_group_details[0].version_group.name == "red-blue") {
          if (
            element.version_group_details[0].move_learn_method.name == "machine"
          ) {
  ...
  return (
    <div className="pokemon_info_cont">
      {pokemon.stats.length > 5 ? (
        <div className="">
          <h1 className="pokemon_displayer_name">
            {params.id.charAt(0).toUpperCase() + params.id.slice(1)}
          </h1>
          <div className="pokemon_nav">
            {pokemonbefor.name != "" ? (
              <button
                className="pre_pokemon"
                onClick={() => handleClick_Pokemon(pokemonbefor.name)}
              >
                #{pokemonbefor.id} {pokemonbefor.name}
              </button>
            ) : (
              <div></div>
            )}
            {pokemonafter.name != "" ? (
              <button
                className="next_pokemon"
                onClick={() => handleClick_Pokemon(pokemonafter.name)}
              >
                #{pokemonafter.id} {pokemonafter.name}{" "}
              </button>
            ) : (
              <div></div>
            )}
          ...
          <img
            ...
            src={`https://img.pokemondb.net/artwork/large/${params.id}.jpg`}
          ></img>
          <div className="pokemon_data">
            <h1>Porkédex data</h1>
            <div>
              <p>National №: {pokemon.number}</p>
            </div>
            <div className="type_name">
              types:{" "}
              <div
                onClick={() => handleClick_Type(pokemon.type1)}
                className={`${pokemon.type1} types`}
              >
                {pokemon.type1}
              </div>{" "}
              <div
                onClick={() => handleClick_Type(pokemon.type2)}
                className={`${pokemon.type2} types`}
              >
                {pokemon.type2}
              </div>
            </div>
            <div className="stats">
              <div className="stats_name">hp: {pokemon.stats[0].base_stat}</div>
              <input
                type="range"
                className={`${pokemon.type1}_stats`}
                min="1"
                max="200"
            ...
            {pokemonchain.map((item, i) => {
              return (
                <div>
                  ...
                  {pokemonchainlvl[i] != undefined ? (
                    <div className="chainlvl">
                      <div>lvl {pokemonchainlvl[i]}</div>
            ...
            <table className="moves_display">
              <tbody>
                <tr className="movestotheleft">
                  <td>
                    {" "}
                    <h2 className="pokemon_moves_headers">
                        ...
                        <div className="move_row">
                          <tr>
                            <p style={{ width: "45px" }}>
                              LV:{" "}
                              {
                                pokemon.LV_moves[i].version_group_details[0]
                                  .level_learned_at
                              }
                            </p>
                            <td>
