...
function Type_Display(match) {
  ...
  useEffect(() => {
    async function getPokemon() {
      ...
      for (let i = 0; i < data.pokemon.length; i++) {
        ...
        const elem2 = await Number(
          element.pokemon.url.slice(34, element.pokemon.url.length - 1)
        );
        if (elem2 < 152) {
      ...
      for (let i = 0; i < data.moves.length; i++) {
        ...
        const elem2 = await Number(
          element.url.slice(31, element.url.length - 1)
        );
        if (elem2 < 166) {
  ...
  return (
    <div className="pokemon_info_cont" style={{ paddingTop: "20px" }}>
      <div className={`${params.id} types`}>{params.id}</div>
